...
> **New Foundry UI note**: The left nav in the new Foundry experience (New Foundry toggle ON) shows Agents, Models, Fine-tune, Tools, Knowledge, Data, Evaluations, Guardrails. There is no "Management" section — that was the classic Hub+Project experience. Connected resources are accessed via **Project details**:

1. Open [ai.azure.com](https://ai.azure.com) → your Agency project
2. Click the **project name** in the top breadcrumb to open a dropdown, then select **Project details**
3. In Project details, locate **Connected resources** → **+ Add connection**
4. Select **Azure OpenAI**
5. Enter the NWRDC AOAI endpoint: `https://nwrdc-aoai-prod.openai.azure.com/`
6. Set authentication to **API Key**, enter `PLACEHOLDER_REPLACED_IN_CODE` as the key
7. Name the connection `nwrdc-aoai-connection` and save
...
**Steps**:
1. Verify the connection exists via the REST API or portal: project breadcrumb → **Project details** → **Connected resources**
2. If missing, re-add it using the portal or REST API steps from Phase B4
3. Confirm `nwrdc-aoai-connection` appears in the Connected resources list
...
## Appendix B — Required Administrative Permissions

This appendix identifies every step that requires elevated privileges, the minimum role needed, and which team is responsible. Use this as input for change management, access request, and security review processes.

### NWRDC Tenant

| Step | Plan ref | Required role | Role type | Who raises the request |
|---|---|---|---|---|
| Deploy AOAI resource and model (Bicep) | A1 | **Contributor** on the target resource group | Azure RBAC | NWRDC platform team |
| Apply RBAC assignment to AOAI resource (Bicep or CLI) | A1 / A3 | **User Access Administrator** (or Owner) on the AOAI resource | Azure RBAC | NWRDC platform team |
| Create app registration | A2 | **Application Administrator** or **Global Administrator** | Entra ID directory role | NWRDC identity team |
| Create service principal (`az ad sp create`) | A2 | **Application Administrator** or **Global Administrator** | Entra ID directory role | NWRDC identity team |
| Configure federated identity credential on app registration | A2 | **Application Administrator** or owner of the specific app registration | Entra ID directory role | NWRDC identity team |
| Enable diagnostic settings on AOAI resource | A4 / §5 | **Monitoring Contributor** on the AOAI resource | Azure RBAC | NWRDC platform team |

> **Note**: Steps A2 (app registration, service principal, federated credential) are the only steps requiring an Entra ID **directory role**. All other NWRDC steps use Azure subscription RBAC and can be delegated without directory admin involvement.
...
| Step | Plan ref | Required role | Role type | Who raises the request |
|---|---|---|---|---|
| Create User-Assigned Managed Identity | B1 | **Managed Identity Contributor** on the resource group | Azure RBAC | Agency platform team |
| Assign UAMI to Foundry AIServices account | B2 | **Contributor** on the Foundry AIServices account | Azure RBAC | Agency platform team |
| Assign UAMI to Container App (if applicable) | B2 | **Contributor** on the Container App resource | Azure RBAC | Agency platform team |
| Assign `Search Index Data Reader` RBAC to UAMI | B3 | **User Access Administrator** (or Owner) on the AI Search resource | Azure RBAC | Agency platform team |
| Assign `Search Service Contributor` RBAC to UAMI | B3 | **User Access Administrator** (or Owner) on the AI Search resource | Azure RBAC | Agency platform team |
| Create project connection via REST API | B4 | **Contributor** on the Foundry AIServices account | Azure RBAC | Agency platform team |

> **Note**: No Entra ID directory roles are required in the Agency tenant. All Agency steps use Azure subscription RBAC only.

---

### Minimum Permission Summary

| Persona | Minimum Azure RBAC | Minimum Entra ID role | Tenant |
|---|---|---|---|
| NWRDC platform engineer | Contributor + User Access Administrator (scoped to AOAI resource group) | None | NWRDC |
| NWRDC identity admin | None (Azure RBAC not needed) | Application Administrator | NWRDC |
| Agency platform engineer | Contributor + User Access Administrator (scoped to Foundry + Search resources) | None | Agency |
| Agency developer | No elevated permissions required | None | Agency |

**Key principle**: Global Administrator is never required. The only directory-level role needed is **Application Administrator** in the NWRDC tenant, scoped to the app registration task (A2) only. After that one-time setup, all ongoing operations (RBAC changes, model updates, credential rotation) use Azure RBAC roles with no directory admin dependency.
